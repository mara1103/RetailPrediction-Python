...
def test_imports():
    ...
    if failed:
        print(f"\nâŒ Failed to import: {', '.join(failed)}")
...
def test_data_loading():
    ...
    csv_paths = [
        "../data_raw/date_24_art_portfolio_20.csv",
        "../../data_raw/date_24_art_portfolio_20.csv",
        "c:/Users/Mara/disertatie/data_raw/date_24_art_portfolio_20.csv",
    ]
    ...
    for csv_path in csv_paths:
        try:
            ...
            print(f"  âœ… Loaded from: {csv_path}")
            ...
            print(f"  âŒ Error loading {csv_path}: {str(e)}\n")
    
    print("âŒ Could not load CSV file from any expected path\n")
    return False


def test_preprocessing():
    """Test preprocessing functions"""
    print("ðŸ” Testing preprocessing functions...")
    ...
    try:
        ...
        # Load data
        csv_path = "../data_raw/date_24_art_portfolio_20.csv"
        df, target = load_and_prepare_data(csv_path)
        ...
        if article_id:
            ...
            print(f"  âœ… Article filtering: {len(df_article)} rows")
        
        # Test missing dates handling
        df_filled = handle_missing_dates(df_article, fill_method='zero')
        print(f"  âœ… Missing dates filled: {len(df_filled)} rows")
        
        # Test scaler
        scaler = TimeSeriesScaler()
        data = df_article[target].values.reshape(-1, 1)
        data_scaled = scaler.fit_transform(data)
        print(f"  âœ… Data scaled: min={data_scaled.min():.4f}, max={data_scaled.max():.4f}")
...
def test_xgboost_model():
    ...
    print("ðŸ” Testing XGBoost model...")
...
def main():
    ...
    for test_name, test_func in tests:
        try:
            ...
            print(f"âŒ {test_name} failed with exception: {str(e)}\n")
    ...
    if passed == total:
        print("ðŸŽ‰ All tests passed! Ready to run: streamlit run app.py\n")
        return 0
    else:
